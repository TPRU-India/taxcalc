...
(wtd_tax_clp['ref_99'], wtd_tax_ref['ref_99'], wtd_tax_diff['ref_99'],
 wtd_tot['ref_99'] = run_calculator(reform_overall, ref_num=99,
                                    reform_desc=reform_desc_combined)
...
"""
# Using stacking process, we first use one reform, then add subsequent reforms
# print('**********************************************')
...
for i in range(ref_num):
    ...
    ref = 'ref_' + str(i)
    (wtd_tax_clp[ref], wtd_tax_ref[ref],
     wtd_tax_diff[ref]) = run_calculator(reform_dict)
    for year in range(START_YEAR, END_YEAR+1):
        if (year >= BASE_YEAR):
            print('******')
            print(f'Current Law:', end=' ')
            print(f' Tax Collection in Rs. Cr. for {year}:', end=' ')
            print(f'{ind_currency(wtd_tax_clp[ref][year] * 1e-7)}')
            print(f'Reform-> \n{reform_desc_ref}Reform:', end=' ')
            print(f' Tax Collection in Rs. Cr. for {year}:', end=' ')
...
for i in range(ref_num):
    ref_tag = 'ref_' + str(i)
    d = dict((k, v)
             for k, v in wtd_tax_diff[ref_tag].items() if k >= BASE_YEAR)
...
plt.suptitle('Composition of the Changes to Total Tax Collection', fontsize=16,
             fontweight="bold")
