...
def ind_currency(curr):
    curr_str = format_currency(curr, 'INR',
                               locale='en_IN').replace(u'\xa0', u' ')
    ...
    if (year >= BASE_YEAR):
        ...
        for output_in_averages in [False, True]:
            ...
            if output_in_averages:
                ...
                print('***** Distribution Tables for ', end=' ')
                print(f'Total Tax Collection (in Rs. crores) for {year} *****')
                # pd.options.display.float_format = '{:,.0f}'.format
                pd.options.display.float_format = '{:.0f}'.format
            ...
            if output_in_averages:
                filename = 'dist-table-all-clp-avg-'+str(year)+'.txt'
                with open(filename, 'w') as dfile:
                    dt1.to_string(dfile)
                filename = 'dist-table-all-ref-avg-'+str(year)+'.txt'
                with open(filename, 'w') as dfile:
                    dt2.to_string(dfile)
                # print text version of each partial distribution table to file
                to_include = ['weight', 'GTI', 'TTI', 'pitax']
                filename = 'dist-table-part-clp-avg-'+str(year)+'.txt'
                with open(filename, 'w') as dfile:
                    dt1.to_string(dfile, columns=to_include)
                filename = 'dist-table-part-ref-avg-'+str(year)+'.txt'
                with open(filename, 'w') as dfile:
                    dt2.to_string(dfile, columns=to_include)
            else:
                filename = 'dist-table-all-clp-total-'+str(year)+'.txt'
                with open(filename, 'w') as dfile:
                    dt1.to_string(dfile)
                filename = 'dist-table-all-ref-total-'+str(year)+'.txt'
                with open(filename, 'w') as dfile:
                    dt2.to_string(dfile)
                # print text version of each partial distribution table to file
                to_include = ['weight', 'GTI', 'TTI', 'pitax']
                filename = 'dist-table-part-clp-total-'+str(year)+'.txt'
                with open(filename, 'w') as dfile:
                    dt1.to_string(dfile, columns=to_include)
                filename = 'dist-table-part-ref-total-'+str(year)+'.txt'
                with open(filename, 'w') as dfile:
                    dt2.to_string(dfile, columns=to_include)
...
# for all years
df_pit_total_clp = df[df.columns[df.columns.str.startswith('pitax_total_clp')]]
df_pit_total_ref = df[df.columns[df.columns.str.startswith('pitax_total_ref')]]
clp_pitax_list = df_pit_total_clp.loc['ALL'].tolist()
# clp_pitax_list = [float(i.replace(',','')) for i in clp_pitax_list]
clp_pitax_list = [round(elem, 0) for elem in clp_pitax_list]
ref_pitax_list = df_pit_total_ref.loc['ALL'].tolist()
# ref_pitax_list = [float(i.replace(',','')) for i in ref_pitax_list]
ref_pitax_list = [round(elem, 0) for elem in ref_pitax_list]
years = [x[-4:] for x in list(df_pit_total_clp.columns)]
...
"""
plt.plot(years, clp_pitax_list, linestyle='-', marker='o', color='b',
         label='Current Law', linewidth=2.0)
...
df_diff_ref = df[df.columns[df.columns.str.startswith('pitax_diff_total_ref')]]
pitax_diff_ref_list = df_diff_ref.loc['ALL'].tolist()
# ref_pitax_diff_list = [float(i.replace(',','')) for i in ref_pitax_diff_list]
pitax_diff_ref_list = [round(elem, 0) for elem in pitax_diff_ref_list]
years = [x[-4:] for x in list(df_diff_ref.columns)]
...
ax.bar(x_pos, pitax_diff_ref_list, width,
       color='orange')
ax.set_xticks(x_pos)
...
ax.set_xlabel('Years')
ax.invert_yaxis()
ax.set_title('Change in Total Tax Burden Due to Reform in 2020',
             fontweight="bold")
...
plt.suptitle('Contribution by Income Bracket to total PIT, 2020 - Current Law',
             fontsize=16, fontweight="bold")
plt.savefig('Contribution to total PIT.png')
plt.show()

# generating pie chart for comparing contribution of tax by different
# income groups for clp and reform for 2020 - the first year of reform
...
plt.suptitle('Contribution by Income Bracket to total PIT, 2020', fontsize=16,
             fontweight="bold")
