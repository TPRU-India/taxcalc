...
dumpdf['percent change in tax'] = np.where(dumpdf['pitax_diff'] == 0.0,
                                           0.0, (dumpdf['pitax_diff']/dumpdf['pitax1'] * 100))
...
# converting the result into deciles and getting mean of three variables each year
df1 = dumpdf.groupby(pd.qcut(dumpdf.GTI, 10))['pitax1', 'pitax2', 'pitax_diff'].mean()
...
df2 = dumpdf.groupby(pd.qcut(dumpdf.GTI, 10))['pitax1', 'pitax2', 'pitax_diff'].sum()
df2 = df2.reset_index()
